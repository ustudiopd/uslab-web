# USLab 웹 시스템 전체 이식 명세서 v1.0

> **목적**: 노블sh 기반 USLab 웹 시스템을 ustudio 웹페이지에 이식 가능한 수준의 전체 명세서  
> **대상**: 블로그 포스팅, 포트폴리오 게시판, 어드민 대시보드, SEO 관리 시스템  
> **작성일**: 2025-01-XX
...
4. [블로그 포스팅 시스템](#4-블로그-포스팅-시스템)
5. [포트폴리오 게시판 시스템](#5-포트폴리오-게시판-시스템)
6. [어드민 대시보드 시스템](#6-어드민-대시보드-시스템)
7. [SEO 관리 시스템](#7-seo-관리-시스템)
8. [인증 및 권한 관리](#8-인증-및-권한-관리)
9. [트래킹 및 분석 시스템](#9-트래킹-및-분석-시스템)
10. [API 명세](#10-api-명세)
11. [프론트엔드 컴포넌트 구조](#11-프론트엔드-컴포넌트-구조)
12. [이식 가이드](#12-이식-가이드)
...
- **프로젝트명**: USLab 웹 시스템
- **기반 프레임워크**: Next.js 16 (App Router)
...
- **에디터**: 
  ...
  - Tiptap Extensions (starter-kit, markdown, html, image, link, youtube 등)
...
#### AI 기능
- **AI SDK**: Vercel AI SDK (`ai` 패키지)
- **모델**: Google Gemini 2.0 Flash (`@ai-sdk/google`)
- **기능**: 
  - AI 이어쓰기
  - AI 교정
  - SEO 메타데이터 자동 생성
...
```
┌─────────────────┐
│  Client Layer   │  Next.js Frontend (React Components)
│  (UI/UX)        │
└────────┬────────┘
         │
┌────────▼────────┐
...
#### 다중 프로젝트 스키마 분리 전략
- **Prefix 기반 분리**: `uslab_`, `ustudio_`, `modu_` 등
- **같은 Supabase 프로젝트 내에서 완전 분리**
- **테이블, 함수, 트리거, RLS 정책 모두 prefix 사용**
...
CREATE TABLE uslab_posts (
  ...
  slug TEXT NOT NULL,                    -- URL 경로
  title TEXT NOT NULL,                   -- 제목
  content JSONB NOT NULL,                -- Tiptap JSON 콘텐츠
  thumbnail_url TEXT,                    -- 썸네일 이미지 URL
  
  -- 다국어 지원
  locale TEXT DEFAULT 'ko' CHECK (locale IN ('ko', 'en')),
  canonical_id UUID REFERENCES uslab_posts(id),  -- 언어별 연결
...
CREATE TABLE uslab_sessions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_key TEXT NOT NULL UNIQUE,      -- 클라이언트 세션 키
  landing_path TEXT,                      -- 첫 유입 경로
  referrer TEXT,                         -- 리퍼러 URL
  referrer_host TEXT,                    -- 리퍼러 호스트
  utm_source TEXT,                       -- UTM 소스
  utm_medium TEXT,                       -- UTM 미디엄
  utm_campaign TEXT,                     -- UTM 캠페인
  utm_term TEXT,                         -- UTM 터미널
  utm_content TEXT,                      -- UTM 콘텐츠
...
#### 1. 작성 페이지 (`/admin/posts/write`)

**주요 기능:**
- 제목 입력
- Slug 입력 (AI 자동 생성 옵션)
- 언어 선택 (ko/en)
- Novel.sh 에디터로 콘텐츠 작성
- 썸네일 이미지 업로드
- 초안 저장 / 발행

**발행 시 자동 처리:**
1. SEO 메타데이터 자동 생성 (`/api/ai/seo`)
2. 버전 스냅샷 저장 (`uslab_post_versions`)
3. `is_published = true`, `published_at` 설정
...
**프롬프트 예시:**
```
제목을 기반으로 SEO 친화적인 영문 slug를 생성해주세요.
- 소문자만 사용
- 하이픈(-)으로 단어 구분
- 특수문자 제거
- 50자 이내
...
// Response
{
  "seo_title": "SEO 최적화된 제목 (60자 이내)",
  "seo_description": "SEO 최적화된 설명 (160자 이내)",
...
- seo_title: 60자 이내, 검색 최적화된 제목
- seo_description: 160자 이내, 검색 결과에 표시될 설명
- seo_keywords: 5-10개의 관련 키워드 배열
...
1. **발행 시**: 자동 저장
2. **AI 교정 전**: 교정 전 버전 저장
3. **수동 스냅샷**: 관리자가 명시적으로 저장
...
- **원문 생성 시**: `canonical_id = id` (self 참조, 트리거로 자동 설정)
...
포트폴리오 게시판은 블로그 시스템을 기반으로 확장 가능합니다.

#### 데이터베이스 스키마 확장

```sql
-- 옵션 1: 블로그 테이블 재사용 (카테고리 필드 추가)
ALTER TABLE uslab_posts ADD COLUMN category TEXT;
-- 'blog' | 'portfolio' 구분

-- 옵션 2: 별도 포트폴리오 테이블 생성
CREATE TABLE uslab_portfolio (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  slug TEXT NOT NULL,
  title TEXT NOT NULL,
  content JSONB NOT NULL,              -- Novel.sh 에디터 콘텐츠
  thumbnail_url TEXT,
  client_name TEXT,                    -- 클라이언트명
  project_type TEXT,                   -- 프로젝트 유형
  technologies TEXT[],                 -- 사용 기술 스택
  project_url TEXT,                    -- 프로젝트 URL
...
블로그 작성 페이지와 동일한 구조를 사용하되, 추가 필드만 추가:
...
├─────────────────────────────────────────┤
│  트래픽 추이 차트 (7일/30일 토글)        │
│  [Line Chart: 일별 페이지뷰/방문자]      │
├─────────────────────────────────────────┤
│  Top 콘텐츠 / Top 페이지 / Top Referrer │
│  [테이블 3열 또는 2행 구성]              │
├─────────────────────────────────────────┤
│  최근 활동                               │
...
const seoStatus = {
  technicalSeo: {
    sitemap: await checkSitemapExists(),      // /sitemap.xml 존재 여부
    robots: await checkRobotsExists(),         // /robots.txt 존재 여부
    canonical: await checkCanonicalTags(),    // canonical 태그 사용 여부
    jsonLd: await checkJsonLd(),              // JSON-LD 구조화 데이터 존재 여부
  },
  contentSeo: {
    missingSeoTitle: await countMissingSeoTitle(),
    missingSeoDescription: await countMissingSeoDescription(),
    longSeoTitle: await countLongSeoTitle(),  // 60자 초과
    longSeoDescription: await countLongSeoDescription(), // 160자 초과
  },
};
```
...
- limit: number (기본값: 10)
- locale: 'ko' | 'en' (선택)
- all: boolean (기본값: false, 관리자만 true 가능)
...
모든 `uslab_` prefix를 원하는 prefix로 변경:

1. **마이그레이션 파일 수정**
   - `uslab_` → `yourprefix_` (예: `ustudio_`)

2. **코드 내 prefix 변경**
   ```typescript
   // lib/queries/posts.ts
   .from('uslab_posts')  // → .from('yourprefix_posts')
   ```

3. **타입 정의 변경**
   ```typescript
   // lib/types/blog.ts
   // UslabPost → YourPrefixPost
   ```

#### 다국어 지원 확장

현재 `ko`, `en`만 지원하지만, 추가 언어 지원 가능:
...
ALTER TABLE uslab_posts 
  ...
  CHECK (locale IN ('ko', 'en', 'ja', 'zh'));  -- 일본어, 중국어 추가
```
...
1. **Supabase 프로젝트 구조**
   - 같은 Supabase 프로젝트 내에서 여러 웹사이트를 운영하는 경우, prefix로 완전 분리 필수
   - RLS 정책도 prefix로 구분
...
4. **트래킹 데이터 Retention**
   - 90일 자동 삭제 함수 (`uslab_cleanup_old_tracking`) 정기 실행 필요
   - Vercel Cron 또는 Supabase Scheduled Triggers 사용
